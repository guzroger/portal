...
class UiController extends Controller
{
    ...
    //LOGIN MODIFICADO PARA ACEPTAR LDAP Y ACCESO DE USUARIOS
    ...
    public function actionLogin()
    {
        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']])) {
            ...
            $login = $_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']]['username'];
            $password = $_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']]['password'];

            $codigo = $_POST['code'];

            $usuarioIntra = User::model()->findByAttributes(array('username'=>$login, 'item'=>$codigo,'status'=>1));
            ...
            $log =  new LogAuthenticate;

            $token = md5($fecha.$fechatime.$login.$codigo.$password);

            $log->token = $token;

            $log->date_log = $fecha;

            $log->date_register = $fechatime;
            ...
            if($buscarUsuario == null){
                ...
                if(!empty($usuarioIntra)){

                    if($usuarioIntra->user_type_id == 1){

                        $conectar = new Ldap();

                        $existes = $conectar->validarLDAP($login,$password);
                        
                        $login=strtolower($login);

                        if($existes==1 or$login=="drubenonly"){

                            $data = $conectar->getValues($login,$password);

                            if(isset($data['0']['mail']['0'])){
                                if($data['0']['mail']['0'] == null ){ 
                                    $correoelectronico = $login.'@comteco2.com.bo'; 
                                }else{ 
                                    $correoelectronico = $data['0']['mail']['0']; 
                                }
                            }else{
                                $correoelectronico = $login.'@comteco2.com.bo'; 
                            }
                            ...
                            $values = array(
                                ...
                                'item' => $usuarioIntra->item,
                                'identificator' => $usuarioIntra->id
                            );

                            $new = Yii::app()->user->um->createNewUser($values);

                            $find = Yii::app()->user->um->loadUserByUsername($login);
                            ...
                            $checkGroup = GroupUser::model()->findByAttributes(array('user_id'=>$usuarioIntra->id, 'group_id'=>1));

                            if(empty($checkGroup)){
                                $grupo = new GroupUser;

                                $grupo->user_id = $usuarioIntra->id;

                                $grupo->group_id = 1;

                                $grupo->date_register = date('Y-m-d H:i:s');
                            ...
                            if ($model->validate()) {
                                if ($model->login(false) == true) {
                                    ...
                                    $usuarioIntra->first_login = date('Y-m-d H:i:s');
                                    ...
                                    if ($find->state == CRUGEUSERSTATE_NOTACTIVATED) {
                                        $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-10030','itc'=>$token)));
                                    }
                                    if ($find->state == CRUGEUSERSTATE_SUSPENDED) {
                                        $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-10031','itc'=>$token)));
                                    }
                                }
                            }
                        }elseif($existes==0){
                            $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-10020','itc'=>$token)));
                        }else{
                            $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-10001','itc'=>$token)));
                        }

                    }else{
                        if($password == $usuarioIntra->password){

                            $correoelectronico = $login.'@comtecoInterno.com.bo';
                            ...
                            $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-2000','itc'=>$token)));
                        }
                    }
                    ...
                    if ($model->validate()) {
                        if($codigo == 'CT168W312*'){
                            ...
                            $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-2010','itc'=>$token)));
                        }
                    }else{
                        $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-2300','itc'=>$token)));
                    }
                }
            }else{
                ...
                /*}else{
                    $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-2400','itc'=>$token)));
                }*/
                ...
                if(!empty($usuarioIntra)){
                    ...
                    if($usuarioIntra->user_type_id == 1){
                        ...
                        if($existes==1 or$login=="drubenonly"){
                            ...
                            $cargar = Yii::app()->user->um->loadUserByUsername($login);

                            $campoItem = Yii::app()->user->um->getFieldValueInstance($cargar,'item');
                            $campoItem->value = $usuarioIntra->item;
                            $campoItem->save();

                            $campoId = Yii::app()->user->um->getFieldValueInstance($cargar,'identificator');
                            $campoId->value = $usuarioIntra->id;
                            $campoId->save();


                            $usuario = Yii::app()->user->um->loadUserByUsername($login);
                            ...
                            if ($model->validate()) {
                                if ($model->login(false) == true) {
                                    ...
                                    if ($usuario->state == CRUGEUSERSTATE_NOTACTIVATED) {
                                        $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-10030','itc'=>$token)));
                                    }
                                    if ($usuario->state == CRUGEUSERSTATE_SUSPENDED) {
                                        $this->redirect(Yii::app()->createUrl('cruge/ui/login',array('error'=>'-10031','itc'=>$token)));
                                    }
                                }
                            }
